...
class KinesisReadable extends Readable {
  ...
  readShard (shardIterator/*: string */) {
    ...
    return this.retryAWS('getRecords', params)
      .then((data) => {
      ...
      return this.client[funcName](params).promise()
        .catch((err) => {
          if (err.retryable === false) {
            throw err // readShard will catch this and emit the error
          }

          retry(err)
        })
    }) // WISHLIST allow user to control retry options https://github.com/IndigoUnited/node-promise-retry
  }
}
